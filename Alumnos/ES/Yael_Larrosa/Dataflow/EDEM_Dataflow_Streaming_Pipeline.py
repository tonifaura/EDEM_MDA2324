...
class TransformCoordinatesToString(beam.DoFn):
    def process(self, element):
        if 'coordinates' in element and isinstance(element['coordinates'], list):
            element['coordinates'] = str(element['coordinates'])
        yield element
    ...
    with beam.Pipeline(argv=pipeline_opts,options=options) as p:
        ...
        (
            processed_data.fined_vehicles
                ...
                | "Format Coordinates" >> beam.ParDo(TransformCoordinatesToString())
                | "Write to BigQuery" >> beam.io.WriteToBigQuery(
                table = "proud-spring-402916:edem_mda.fined_vehicles2", # Required Format: PROJECT_ID:DATASET.TABLE
