...
class avgSpeedDoFn(beam.DoFn):
    ...
    def process(self, element):
        ...
        output_dict = {
            ...
            "vehicle_id": key,
            "avg_speed": avg_speed,
            "coordinates": str(payload[-1]['location']) # line ----> string
        }
...
def run():
    ...
    parser.add_argument(
                '--project_id',
                required=False,
                default='awesome-ridge-411708',
                help='GCP cloud project name.')
    
    parser.add_argument(
                '--input_subscription',
                default='projects/awesome-ridge-411708/subscriptions/input-sub',
                required=False,
                help='PubSub subscription from which we will read data from the generator.')
    
    parser.add_argument(
                '--output_topic',
                default='projects/awesome-ridge-411708/topics/output',
                required=False,
    ...
    with beam.Pipeline(argv=pipeline_opts,options=options) as p:
        ...
        (
            processed_data.fined_vehicles
                ...
                | "Write fined_vehiculos to BigQuery" >> beam.io.WriteToBigQuery(
                table = "awesome-ridge-411708:datasetedem.dataflow-car", # Required Format: PROJECT_ID:DATASET.TABLE
        ...
        (
            processed_data.non_fined_vehicles 
                | "Write no_fined_vehiculos to BigQuery" >> beam.io.WriteToBigQuery(
                    table = "awesome-ridge-411708:datasetedem.dataflow-car", # Required Format: PROJECT_ID:DATASET.TABLE
                    schema='radar_id:STRING, vehicle_id:STRING, avg_speed:FLOAT, coordinates:STRING, is_Ticketed:BOOLEAN, license_plate:STRING, image_url:STRING',  # Required Format: field:TYPE
                    create_disposition=beam.io.BigQueryDisposition.CREATE_NEVER,
                    write_disposition=beam.io.BigQueryDisposition.WRITE_APPEND
