...
class OutputFormatDoFn(beam.DoFn):
    def process(self, element):
        ...
        # No se intenta extraer la placa de licencia; simplemente se devuelve el output_dict como está.
        yield output_dict
    ...
    with beam.Pipeline(argv=pipeline_opts,options=options) as p:
        ...
        (
            processed_data.fined_vehicles \
                ...
                | "Print Element multa" >> beam.Map(lambda element: (print("Element before Encode", element)) or element)
                | "Write fined_vehicles to PubSub" >> beam.io.WriteToPubSub(topic="projects/principal-iris-416209/topics/output_multas")

        )
        (
            processed_data.non_fined_vehicles 
                | "Encode non_fined_vehicles to Bytes" >> beam.Map(lambda x: json.dumps(x).encode("utf-8"))
                | "Print Element no multas" >> beam.Map(lambda element: (print("Element before Encode", element)) or element)
                | "Write non_fined_vehicles to PubSub" >> beam.io.WriteToPubSub(topic="projects/principal-iris-416209/topics/ouput_no_multas")
        )
